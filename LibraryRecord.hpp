...
class LibraryRecord : public ArrayBag<Book>{
	 ...
	 */
	bool checkOut(const Book &new_checked_out);
	/**
...
bool equivalentRecord(LibraryRecord &library);
...
LibraryRecord operator+=(LibraryRecord &library);
...
private:
	std::vector<Book> checked_out_;//A vector containing a copy of all the Books that have been checked out.
};
