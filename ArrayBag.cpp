...
bool ArrayBag<ItemType>::contains(const ItemType& an_entry) const
{
 ...
 /**
   * @param: array  A reference to another ArrayBag object
   ...
   */
template<class ItemType>
ArrayBag<ItemType> ArrayBag<ItemType>::operator+=(ArrayBag array){
   ArrayBag<ItemType> tmp;
   int i=0;

   for(i=0;i<item_count_;i++)
      tmp.items_[i]=this->items_[i];

   while(i<DEFAULT_CAPACITY && i< array.item_count_){
    tmp.items_[i]=array->items_[i];
    i++;
   }
   return tmp;
}
